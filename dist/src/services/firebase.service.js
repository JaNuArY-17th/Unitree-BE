...
let FirebaseService = FirebaseService_1 = class FirebaseService {
    ...
    constructor(configService) {
        ...
        if (!firebaseConfig.projectId) {
            this.logger.warn('Firebase configuration is missing (projectId is empty). Firebase features will be disabled.');
            return;
        }
    ...
    async sendMulticast(fcmTokens, title, body, data) {
        if (!this.firebaseApp) {
            this.logger.warn('Firebase is not initialized. Cannot send multicast notification.');
