...
export class CreateWifiConfigAndUpdateWifiSession20260404130000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TYPE "wifi_session_status" ADD VALUE IF NOT EXISTS 'timeout'`,
    );
    await queryRunner.query(
      `ALTER TYPE "wifi_session_status" ADD VALUE IF NOT EXISTS 'cheat_flagged'`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    const foreignKey = table?.foreignKeys.find(
      (fk) => fk.columnNames.indexOf('wifi_config_id') !== -1,
    );
    if (foreignKey) {
