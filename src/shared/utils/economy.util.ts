export class EconomyUtil {
  ...
  static calculateOxygenHarvest({
    ...
    if (lastHeartbeat && lastHeartbeat > lastHarvestTime) {
      ...
      const wifiSeconds = Math.max(
        0,
        (wifiEndTime.getTime() - lastHarvestTime.getTime()) / 1000,
      );
      const wifiOxygen = this.applyWifiBoost(
        currentYield * (wifiSeconds / 3600),
      );
      totalOxygen += wifiOxygen;

      if (lastHeartbeat < now) {
        const noWifiSeconds = Math.max(
          0,
